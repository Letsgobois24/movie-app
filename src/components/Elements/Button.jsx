const Button = ({children, color, onClick, className=''}) => {
    ...
    return (
        <button onClick={onClick} className={`${classColor[color]} ${className} inline-flex cursor-pointer justify-center items-center py-3 px-5 text-base font-medium text-center rounded-lg focus:ring-4 shadow-md transition`}>
            {children}
        </button>
    )
}
